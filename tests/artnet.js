   ...
   ========================================================================== */
{
  const name = 'docs/ARTNET.md is exactly what tools/artnet-map.js emits now, ' +
               'and its blocks tile every channel with no gaps (RULINGS EO, FD)';
  ...
  try{
    ...
    /* RULING FD — THE BLOCKS TILE THE CHANNEL RANGE.  FA's scan asked that
       every line begin with a channel number; FD groups the sheet, so that
       scan cannot survive.  What it was FOR does survive, and this is it:
       every channel must fall inside exactly one block, so a ruling that adds
       a channel and forgets to file it fails here rather than shipping a sheet
       that quietly omits it.

       Parsed with indexOf and a digit walk, NOT a regex — TRAPS: a backslash
       in a regex is eaten by a probe template, and this case has already been
       bitten once by trusting a pattern where an exact scan was available. */
    const HEAD = '== ', MID = ' ==  channels ';
    const blocks = [];
    for(let i = 4; i < want.length; i++){
      const line = String(want[i]);
      if(line.indexOf(HEAD) !== 0) continue;
      const cut = line.indexOf(MID);
      if(cut < 0)
        throw new Error('docs/ARTNET.md has a block heading that does not declare its channel ' +
          'range, which is the whole of RULING FD: ' + JSON.stringify(line.slice(0, 70)));
      const title = line.slice(HEAD.length, cut);
      const span = line.slice(cut + MID.length);
      const dash = span.indexOf('-');
      if(dash <= 0)
        throw new Error('docs/ARTNET.md block ' + JSON.stringify(title) +
          ' does not declare a FROM-TO range: ' + JSON.stringify(span.slice(0, 40)));
      const numOf = s=>{
        if(!s.length) return null;
        let n = 0;
        for(const c of s){ if(c < '0' || c > '9') return null; n = n * 10 + (c.charCodeAt(0) - 48); }
        return n;
      };
      const from = numOf(span.slice(0, dash)), to = numOf(span.slice(dash + 1));
      if(from === null || to === null)
        throw new Error('docs/ARTNET.md block ' + JSON.stringify(title) +
          ' has a non-numeric channel range: ' + JSON.stringify(span.slice(0, 40)));
      blocks.push({title: title, from: from, to: to, line: i + 1});
    }
    if(!blocks.length)
      throw new Error('docs/ARTNET.md has no "== BLOCK ==  channels A-B" headings at all — ' +
        'RULING FD is a grouped patch sheet, and nothing in it declares a range');
    let due = 1;
    for(const b of blocks){
      if(b.to < b.from)
        throw new Error('docs/ARTNET.md block ' + JSON.stringify(b.title) + ' runs backwards, ' +
          b.from + '-' + b.to + ' at line ' + b.line);
      if(b.from !== due)
        throw new Error('docs/ARTNET.md block ' + JSON.stringify(b.title) + ' starts at channel ' +
          b.from + ' where ' + due + ' was due (line ' + b.line + ') — ' + (b.from > due
            ? 'channels ' + due + '..' + (b.from - 1) + ' are in no block at all'
            : 'the blocks overlap') + ', and RULING FD is every channel in exactly one block');
      due = b.to + 1;
    }
    const lastCh = due - 1;
